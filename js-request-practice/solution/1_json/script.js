...
function test2() {
  ...
  resultEl.innerHTML = `
    ...
    <strong>활성 여부:</strong> ${user.isActive}<br>
...
function test3() {
  ...
  document.getElementById("section3-json").textContent = JSON.stringify(
    parsed,
    null,
    2,
  );
...
function test4() {
  ...
  const totalPrice = parsed.orders.reduce((sum, order) => sum + order.price, 0);
...
function test5() {
  ...
  const parsedDate = JSON.parse(JSON.stringify(dataWithDate));
  ...
  const parsedFunc = JSON.parse(JSON.stringify(dataWithFunc));
  ...
  const parsedUndef = JSON.parse(JSON.stringify(dataWithUndef));
