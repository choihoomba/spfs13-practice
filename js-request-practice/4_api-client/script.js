...
async function test3() {
  ...
  try {
    // TODO: 위에서 만든 api 객체를 사용하여 CRUD를 실행하세요
    // api.get(endpoint), api.post(endpoint, data), api.put(endpoint, data), api.delete(endpoint)
...
// TODO: 공통 request 함수를 완성하세요
// 모든 HTTP 메서드가 이 함수 하나를 거칩니다
...
// TODO: request 함수를 사용해서 api2 객체를 완성하세요
// 각 메서드가 request()를 호출하도록 만드세요
const api2 = {
...
// TODO: 최종 완성 API 클라이언트
// request 함수 + 상태코드 메시지 + 에러 처리를 모두 합칩니다
...
async function finalRequest(endpoint, options = {}) {
  ...
  if (!response.ok) {
    // TODO: getErrorMessage를 활용하여 친절한 에러를 던지세요
    throw new Error(_____); // getErrorMessage(response.status) 활용
  }
